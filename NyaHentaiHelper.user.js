...
(() => {
  ...
  if (localStorage.getItem('NYAHENTAI_HELPER_DEBUG') && typeof unsafeWindow.N !== 'undefined') {
  ...
  const _log = (...args) => console.log('[nyahentai-helper]', ...args);
  const _warn = (...args) => console.warn('[nyahentai-helper]', ...args);
  const _error = (...args) => console.error('[nyahentai-helper]', ...args);

  class JSZipWorkerPool {
    constructor() {
      this.pool = [];
      this.WORKER_URL = URL.createObjectURL(new Blob(['importScripts("https://unpkg.com/comlink/dist/umd/comlink.js","https://raw.githubusercontent.com/Stuk/jszip/master/dist/jszip.min.js");class JSZipWorker{constructor(){this.zip=new JSZip}file(name,{data:data}){this.zip.file(name,data)}generateAsync(options,onUpdate){return this.zip.generateAsync(options,onUpdate).then(data=>Comlink.transfer({data:data},[data]))}}Comlink.expose(JSZipWorker);'], {
    ...
    async generateAsync(files, options, onUpdate) {
      ...
      for (const {
        ...
        await zip.file(name, Comlink.transfer({
          data
        }, [data]));
      }
      return zip.generateAsync(options, onUpdate).then(({
        data
      }) => {
        worker.idle = true;
        return data;
...
9: highest (best compression)`,
  ...
  class AsyncQueue {
    ...
    async start() {
      if (this.thread <= 1) {
        ...
        const idleItems = this.queue.filter(({
          running
        }) => !running);
        for (let i = 0; i < Math.min(idleItems.length, this.thread - running); i++) {
          ...
          item.running = true;
          item.fn().then(() => {
            this.queue.remove(this.queue.findIndex(({
              id
            }) => id === item.id));
            this.start();
          });
    ...
    methods: {
      cancel() {
        if (this.index === 0) {
          ...
          const {
            info
          } = dlQueue.queue.remove(this.index);
  ...
  new Vue({
    ...
    computed: {
      infoList() {
        return [...this.zipQueue, ...this.dlQueue].map(({
          info
        }) => info);
      },
    },
  ...
  const downloadGallery = async ({
    ...
    pages,
    cfName
  }, $btn = null, $btnTxt = null, headTxt = false) => {
    ...
    const dlPromise = (page, threadID) => {
      ...
      const url = CUSTOM_DOWNLOAD_URL ? getTextFromTemplate(CUSTOM_DOWNLOAD_URL, {
        mid: mid,
        index: page.i,
        ext: page.t
    ...
    if (pageType.gallery) {
      ...
      $('.gallery').each(function() {
        ...
        $btn.click(async () => {
          ...
          if (downloaded || dlQueue.queue.some(({
              info: {
                title
              }
            }) => title === gallery.title)) {
          ...
          dlQueue.push(
            async () => {
              const {
                zipFn,
                zipInfo
              } = await downloadGallery(gallery, $btn, $btnTxt);
              if (zipInfo) {
                zipQueue.push(async () => {
                  const {
                    data,
